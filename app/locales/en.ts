export default {
  nav: { home: 'Home', services: 'Services', about: 'About us', contact: 'Contact' },
  language: 'Language',
  common: {
    whatsapp: 'CHAT ON WHATSAPP',
    contactUs: 'CONTACT US',
    explore: 'Explore',
    viewService: 'View service',
    notFound: 'Service not found.',
  },
  home: {
    eyebrow: 'SOLUTIONS THAT MOVE YOUR BUSINESS',
    title: 'Professional solutions',
    titleAccent: 'for your supply chain.',
    description:
      'We support companies looking to optimize their operations with efficient logistics, sourcing, and consulting solutions.',
    servicesEyebrow: 'OUR SERVICES',
    servicesTitle: 'Solutions tailored to your operation.',
    aboutEyebrow: 'ABOUT US',
    aboutTitle: 'Strategic approach. Real results.',
    aboutDescription:
      'We are a team of professionals experienced in logistics, sourcing, and consulting. We help companies make better decisions, reduce costs, and improve performance throughout their supply chain.',
    aboutVision:
      'Our vision is to be the strategic partner of choice for businesses looking to turn their supply chain into a sustainable competitive advantage.',
    learnMore: 'LEARN MORE ABOUT US',
  },
  cta: { eyebrow: 'DO YOU HAVE A PROJECT IN MIND?', title: 'Let’s talk about how we can help.' },
  services: {
    procurement: {
      eyebrow: 'SERVICE 01',
      title: 'Procurement & Strategic Sourcing',
      description:
        'We optimize sourcing, supplier selection, and negotiation to reduce costs and improve purchasing quality.',
    },
    'planning-cost-control': {
      eyebrow: 'SERVICE 02',
      title: 'Supply Chain Planning & Cost Control',
      description:
        'We plan demand and control operating costs to ensure budget efficiency and financial visibility.',
    },
    'logistics-materials': {
      eyebrow: 'SERVICE 03',
      title: 'Logistics & Materials Management',
      description:
        'We manage inventories, warehouses, and material flows to ensure availability, order, and traceability.',
    },
    'advisory-risk': {
      eyebrow: 'SERVICE 04',
      title: 'Supply Chain Advisory & Risk Management',
      description:
        'We identify risks, analyze markets, and design strategies to strengthen supply-chain resilience.',
    },
  },
  serviceDetail: {
    scope: 'SERVICE SCOPE',
    value: 'VALUE WE CREATE',
    methodology: 'OUR METHODOLOGY',
    methodologyTitle: 'How we work with you',
  },
  footer: {
    navigation: 'NAVIGATION',
    services: 'SERVICES',
    contact: 'CONTACT',
    description: 'Strategic solutions to optimize your supply chain with a focus on real results.',
    rights: 'All rights reserved.',
    privacy: 'Privacy notice',
    terms: 'Terms and conditions',
  },
}
